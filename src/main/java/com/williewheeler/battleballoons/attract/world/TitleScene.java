package com.williewheeler.battleballoons.attract.world;
...
public class TitleScene extends ScriptScene {
	...
	private void generateBalloon(int x0, int yBase, int dxBase, int drot) {
		if (RandomUtil.nextRandomDouble() < CREATE_PROBABILITY) {
	...
	private int generateY(int yBase) {
		int yOffset = RandomUtil.nextRandomInt(BALLOON_Y_RANGE) - BALLOON_Y_RANGE / 2;
		return yBase + yOffset;
	}

	private int generateDx(int dxBase) {
		int dxOffset = RandomUtil.nextRandomInt(BALLOON_DX_RANGE) - BALLOON_DX_RANGE / 2;
